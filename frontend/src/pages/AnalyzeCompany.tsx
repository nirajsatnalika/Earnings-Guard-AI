...
export function AnalyzeCompany() {
  ...
  const updateFile = (index: number, file: File | null) => { setStatements((current) => current.map((statement, statementIndex) => statementIndex === index ? { ...statement, file, progress: file ? 100 : 0 } : statement)) }
  ...
  const canContinue = activeStep === 0 ? Boolean(company.companyName && company.stockSymbol && company.industry && company.country && company.financialYear) : activeStep === 1 ? statements.every(({ file }) => file) : true
...
function UploadStatements({ statements, updateFile }: { statements: UploadedStatement[]; updateFile: (index: number, file: File | null) => void }) {
  return <Box><Typography variant="h2">Upload financial statements</Typography><Typography color="text.secondary" sx={{ mt: .75, mb: 2.5 }}>Upload the latest annual statements. We accept Excel and CSV files only.</Typography><Box sx={{ display: 'grid', gridTemplateColumns: { xs: '1fr', md: 'repeat(3, 1fr)' }, gap: 2 }}>{statements.map((statement, index) => <UploadCard key={statement.type} statement={statement} onFileChange={(file) => updateFile(index, file)} />)}</Box><Alert severity="info" icon={<InfoOutlined />} sx={{ mt: 2 }}>For best results, upload statements from the same financial year.</Alert></Box>
}

function Validation({ missingFields }: { missingFields: string[] }) {
